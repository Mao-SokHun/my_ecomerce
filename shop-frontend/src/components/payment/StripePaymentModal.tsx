...
function CheckoutForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 p-3 bg-white dark:bg-gray-950 min-h-[120px]">
        {!ready && (
          <div className="flex items-center justify-center gap-2 py-8 text-sm text-gray-400">
            <Loader2 className="w-5 h-5 animate-spin" />
            {language === 'km' ? 'កំពុងផ្ទុក...' : language === 'zh' ? '加载中...' : 'Loading...'}
          </div>
        )}
        <PaymentElement
          onReady={() => setReady(true)}
          onLoadError={(e) => setError(e.error?.message || 'Failed to load payment form. Please try again.')}
        />
      </div>
      <button
        type="submit"
