...
export function CustomDropdown({
  ...
  const sizeClasses = {
    xs: 'h-7 text-[11px] px-2.5 rounded-lg gap-1.5',
    sm: 'h-8.5 text-xs px-3 rounded-xl gap-2',
    md: 'h-10 text-sm px-3.5 rounded-xl gap-2.5',
  }[size];
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute z-50 mt-1.5 min-w-[170px] max-h-72 overflow-y-auto bg-white/98 dark:bg-slate-900/98 backdrop-blur-2xl border border-slate-200/90 dark:border-slate-750 rounded-2xl shadow-xl shadow-slate-900/10 dark:shadow-black/60 p-1.5 ring-1 ring-black/5 focus:outline-none ${
              ...
              {options.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-2.5 py-2 text-xs rounded-xl font-medium transition-all duration-150 text-left ${
                      isSelected
